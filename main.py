...
def main() -> None:
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📥 下載原始碼")
    _src_dir = __file__.replace("main.py", "")
    _files   = [
        ("main.py",          "入口路由"),
    ...
    for _fname, _flabel in _files:
        _fpath = _src_dir + _fname
        try:
            import os as _os
            ...
            st.sidebar.download_button(
                ...
                mime="text/x-python",
                use_container_width=True,
                help=f"{_flabel}　{len(_fbytes.splitlines())} 行　{_fsize/1024:.1f} KB",
                key=f"dl_{_fname}",
            )
        except Exception:
            st.sidebar.caption(f"⚠️ {_fname} 暫不可用")
